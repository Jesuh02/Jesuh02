...
<h1>Yasuy | Senior Full Stack Engineer & IA Aplicada</h1>
...
<p>
  ...
  <img alt="Data Systems" src="https://img.shields.io/badge/Data%20Systems-Scraping%20%2B%20Redis%20%2B%20Nginx-b45309?style=for-the-badge">
  <img alt="Architecture" src="https://img.shields.io/badge/Architecture-Hexagonal%20%2B%20Ports%20%26%20Adapters-0f172a?style=for-the-badge">
</p>
...
```mermaid
flowchart LR
  A[Imagen o frame] --> B[YOLOv8 localiza la placa]
  B --> C[Recorte, padding y variantes visuales]
  C --> D[PaddleOCR lee caracteres]
  D --> E[Normalizacion y regex colombiana]
  E --> F[Tracking y buffer temporal]
  F --> G[Registro de deteccion]
  G --> H[Consulta de propietario]
  H --> I[WhatsApp, reporte o asistente LLM]
```
...
sequenceDiagram
  ...
  Docente->>PWA: Describe el curso
  PWA->>IA: Envia prompt y contexto educativo
  IA->>MCP: Solicita herramientas de dominio
  MCP->>API: Construye propuesta estructurada
  API->>PWA: Devuelve plan editable
  Docente->>PWA: Revisa, ajusta y confirma
  PWA->>API: Persiste curso completo
  API->>DB: Guarda materias, temas y tareas
  API->>R2: Publica recursos en background
```
...
```mermaid
flowchart LR
  A[Usuario busca alojamiento] --> B[API FastAPI]
  B --> C[Cache local, disco y Redis]
  C --> D{Hay datos vigentes?}
  D -->|Si| E[Respuesta rapida]
  D -->|No| F[Scrapers por fuente]
  F --> G[Playwright, HTTPX y BeautifulSoup]
  G --> H[Normalizacion y deduplicacion]
  H --> I[Filtrado por precio real]
  I --> J[Mapa, listados y estadisticas]
  H --> C
```
...
<p>
  ...
  <img alt="Vue" src="https://img.shields.io/badge/Vue%203-42B883?style=flat-square&logo=vue.js&logoColor=white">
  ...
  <img alt="Cloudflare" src="https://img.shields.io/badge/Cloudflare%20R2-F38020?style=flat-square&logo=cloudflare&logoColor=white">
